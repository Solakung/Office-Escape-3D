    ...
    // -------------------------------------------------------------
    // Farewell Note (จดหมายลาก่อนของพนักงาน — องก์ 2 เท่านั้น)
    // แยกจาก loreNotes/LORE_TEXTS โดยสิ้นเชิง: ชิ้นเดียว เก็บได้ครั้งเดียวต่อรอบเล่น
    // เก็บแล้วอ่านค้าง 5 วิ -> ไฟดับ -> บังคับหลุดเข้า Deep Backroom
    // -------------------------------------------------------------
    let farewellNoteTriggered = false; // persist ทั้งรอบเล่น กันเก็บ/ทริกเกอร์ซ้ำ
    let farewellSequenceActive = false; // true ระหว่างอ่าน+ไฟดับ+หลุด ล็อค input ทั้งหมด
    window.farewellSequenceActive = false;
    let specialNotes = []; // { mesh, x, z, active, text } — เก็บแค่ชิ้นเดียว ไม่ใช่ array 12 ใบแบบ loreNotes

    const FAREWELL_NOTE_TEXT =
      "ถ้าใครเจอจดหมายนี้... ผมคงไม่อยู่ตรงนี้แล้ว สามคืนที่ผ่านมาผมนับก้าวเดินตัวเอง" +
      "ไม่ตรงกับที่จำได้ทุกครั้ง ประตู EXIT ที่ป้ายบอกทางชัดเจน... ผมเริ่มไม่แน่ใจแล้วว่า" +
      "มันจะพาผมออกไปจริงหรือเปล่า ถ้าคุณอ่านตรงนี้อยู่ แปลว่าไม่มีใครตามหาผมอีกแล้ว" +
      "เหมือนกัน — ผมชื่อ... ผมชื่ออะไรนะ ผมเริ่มจำไม่ได้แล้วด้วยซ้ำ";

    const FAREWELL_NOTE_MIN_DIST = 26; // บังคับให้ห่างจากจุดเข้าตึก ต้องสำรวจจริง
    const FAREWELL_NOTE_MAX_DIST = 50;
    const FAREWELL_READ_MS = 5000;          // ระยะเวลาอ่านบังคับ ~5 วิ
    const FAREWELL_BLACKOUT_MS = 450;       // ไฟดับ/เฟดดำ ต้องไวและกะทันหัน ไม่ใช่ค่อยๆหรี่
    const FAREWELL_HOLD_DARK_MS = 1400;     // ค้างมืดสนิทก่อนเผยภาพ Deep Backroom
    const FAREWELL_SANITY_PENALTY = 15;     // แรงกระแทกทางจิตใจจากการอ่านจดหมายนี้โดยเฉพาะ
